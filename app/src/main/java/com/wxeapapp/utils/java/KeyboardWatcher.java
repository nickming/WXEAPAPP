...
public class KeyboardWatcher {
    ...
    private void initialize() {
        if (hasAdjustResizeInputMode()) {
            ...
            rootViewRef = new WeakReference<>(activityRef.findViewById(Window.ID_ANDROID_CONTENT));
            ...
            throw new IllegalArgumentException(String.format("Activity %s should have windowSoftInputMode=\"adjustResize\"" +
                    "to make KeyboardWatcher working. You can set it in AndroidManifest.xml", activityRef.getClass().getSimpleName()));
        }
    }

    private boolean hasAdjustResizeInputMode() {
        return (activityRef.getWindow().getAttributes().softInputMode & WindowManager.LayoutParams.SOFT_INPUT_ADJUST_RESIZE) != 0;
    }
